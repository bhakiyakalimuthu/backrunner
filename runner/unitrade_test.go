...
func TestUniTrade_ExecuteBackrunFail(t *testing.T) {
	...
	err = u.execute(ctx, trade, tx)
	assert.Equal(t, "sushi pool does not available for this token pair", err.Error())
}
...
func TestUniTrade_ExecuteBackrun(t *testing.T) {
	...
	txHash := common.HexToHash("0x58b8f6c6076d1d3d10f752cb80dcf369b774e0b463723095fec10c6a9124b320")
	tx, _, err := ethClient.TransactionByHash(ctx, txHash)
	if err != nil {
		t.Errorf("failed to TransactionByHash %v", err)
		return
