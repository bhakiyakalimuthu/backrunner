...
// UniTrade extends the Trade interface to perform backrun
...
func (u *UniTrade) ExecuteBackrun(ctx context.Context, txHash common.Hash) {
	...
	u.logger.Info("received pending tx", zap.String("txHash", txHash.Hex()))
	...
	// Back run only uniswap trades for simplicity
	if tx.To().Hash() == _UniSwapRouter02.Hash() {
		u.logger.Info("=============================")
		u.logger.Info("Found uniswap trade", zap.String("txHash", txHash.Hex()))
		u.logger.Info("=============================")
		...
		if trade != nil && receipt != nil && (trade.Params.TokenOut) == _WETHAddress.String() {
			u.logger.Debug("=============================")
			u.logger.Info("Execution trade", zap.String("txHash", txHash.Hex()))
...
func (u *UniTrade) execute(ctx context.Context, trade *btypes.UniswapTrade, mempoolTx *types.Transaction) error {
	...
	if err != nil {
		u.logger.Error("failed to get pending nonce", zap.Error(err))
		return nil
	}
	...
	tx, err := u.contracts.Executor.Execute(auth,
		...
		*uniPoolAddress,
		...
		sushiToken0 == common.HexToAddress(trade.Params.TokenIn),
